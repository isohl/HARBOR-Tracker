			...
			while 1:    #   off of the D710 and attaching a time stamp to it in the file
				...
				if out!="":
					...
					try:
						...
						p = fap.Packet(out)
						if p.altitude:
							point = helper.Point(time=currentTimestamp, altitude=int(p.altitude*3.28084), latitude=p.latitude, longitude=p.longitude)
						...
						if gpscompatible:
							...
							point = helper.Point(time=currentTimestamp, altitude=0, latitude=latlong[1], longitude=latlong[0])
